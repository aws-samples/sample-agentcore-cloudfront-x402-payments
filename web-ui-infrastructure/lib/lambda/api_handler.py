...
def invoke_agent(event):
    ...
    try:
        ...
        # Prepare payload as JSON bytes (matching runtime_client.py)
        ...
        if 'response' in response:
            ...
            # StreamingBody - read all content
            if hasattr(resp, 'read'):
                ...
                try:
                    ...
                    completion = raw
            # Handle streaming response with iter_lines
            elif hasattr(resp, 'iter_lines'):
                lines = []
                for line in resp.iter_lines(chunk_size=10):
                    if line:
                        line_str = line.decode('utf-8')
                        if line_str.startswith('data: '):
                            lines.append(line_str[6:])
                completion = '\n'.join(lines)
